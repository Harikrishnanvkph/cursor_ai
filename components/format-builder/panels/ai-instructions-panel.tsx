...
export function AiInstructionsPanel() {
  ...
  // Determine current mode based on zone contentMode or message
  const mode = baseZone
  ...
  return (
    <ResizablePanelGroup direction="vertical" className="flex flex-col h-full overflow-hidden w-full">
          ...
          <div className="flex-1 overflow-y-auto p-1.5 space-y-1 pr-2">
            ...
            {zones.length === 0 && (
              <p className="text-[11px] text-gray-600 text-center py-6">
                No zones created yet.<br />Go to &apos;Add Zones&apos; tab to add one.
              </p>
            )}
          </div>
        </div>
      ...
      {/* Section 2: Instruct Zone Details */}
      <ResizablePanel defaultSize={65} minSize={30} className="flex flex-col">
        {baseZone ? (
          <div className="p-3 bg-gray-950/20 flex-1 flex flex-col overflow-hidden h-full space-y-3">
            {/* Header Badge */}
            <div className="flex items-center justify-between pb-2 border-b border-gray-900 shrink-0">
              <span className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">
                Instruct Zone
              </span>
              <div className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-semibold uppercase tracking-wider border ${
                mode === 'ai'
            ...
            <div className="flex-1 overflow-y-auto">
              {mode === 'ai' ? (
                ...
                /* Static/Preset is currently selected */
                <div className="p-5 rounded-xl border border-gray-800 bg-[#0c0c14]/80 text-center space-y-4 shadow-xl backdrop-blur-sm mt-3 mx-0.5">
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-[11px] text-gray-400 leading-relaxed max-w-[230px] mx-auto">
                      This zone is currently using static preset content.<br className="hidden sm:block" />
                      To edit manually, go to <span className="text-orange-400 font-semibold">Add Zones → Edit Zone</span>, or click below to instruct AI to generate content.
                    </p>
                  </div>
                  <button
                    onClick={handleEnableAiMode}
          ...
          <div className="p-6 bg-gray-950/10 text-center flex-1 flex flex-col items-center justify-center h-full">
            ...
            <p className="text-[11px] text-gray-500">
              Select a zone from the list above<br />to write AI generation instructions.
            </p>
          </div>
        )}
      </ResizablePanel>
