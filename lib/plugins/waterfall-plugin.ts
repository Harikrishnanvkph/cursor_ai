...
/**
 * Waterfall Plugin — Renders connector lines between consecutive bars in a waterfall chart.
 *
 * It reads configuration from chart.options.plugins.waterfall
 */

export interface WaterfallPluginOptions {
  enabled: boolean;
...
export const waterfallPlugin: Plugin = {
  ...
  afterDatasetsDraw(chart: any, _args: any, pluginOptions: any) {
    ...
    for (let i = 0; i < meta.data.length - 1; i++) {
      const currBar: any = meta.data[i];
      const nextBar: any = meta.data[i + 1];

      if (!currBar || !nextBar) continue;

      if (indexAxis === 'x') {
        // Vertical chart: draw horizontal dashed connector line
        ...
        // Horizontal chart: draw vertical dashed connector line
